  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block truncate text-sm font-semibold text-foreground">{t(`${career.slug}.name`)}</span>
        <span className="mt-0.5 block text-xs text-muted-foreground">{t(`${career.slug}.tagline`)}</span>
      </span>
      <span className="shrink-0 text-right">
        <span className="block text-sm font-bold text-accent">{percent}%</span>
        <span className="block text-2xs text-muted-foreground">{t("matchLabel")}</span>
      </span>
