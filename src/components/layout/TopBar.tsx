...
export function TopBar() {
  ...
  const markRead = useTriply((s) => s.markNotificationRead);
      ...
      <Popover.Root>
        ...
        <Popover.Portal>
          <Popover.Content align="end" sideOffset={10} className="z-50 w-80 rounded-xl border border-border bg-surface p-2 shadow-pop animate-scale-in">
            ...
            <Link href="/inbox" className="mt-1 block rounded-lg px-2.5 py-2 text-center text-[12.5px] font-medium text-accent hover:bg-muted">
              See all in Inbox
            </Link>
          </Popover.Content>
        </Popover.Portal>
