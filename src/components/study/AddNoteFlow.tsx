...
/**
 * The full add-note flow: source picker (upload PDF/photo/image, type notes,
 * paste text) → save → analyze → hand back the new material's id via
 * `onDone`. Extracted from `subjects/[subjectId]/materials/new/page.tsx`
 * unchanged in behavior so both the subject-scoped nested flow and the flat
 * `/app/school/notes/new` entry point share one implementation.
 */
    ...
    <div className="space-y-6">
      {mode === "choose" ? (
        ...
        <div className="space-y-4">
          ...
          <h2 className="text-base font-bold text-foreground">{textKind === "paste" ? t("pasteText") : t("addNotes")}</h2>
          <p className="text-sm text-muted-foreground">
            {textKind === "paste"
              ? t("pasteHint")
              : t("typeHint")}
